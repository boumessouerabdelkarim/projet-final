   ...
   {etab.heure_ouverture&& <h2> Heure d'ouverture:{etab.heure_ouverture}</h2> }
    ...
    <div className="aaaaaa">
      <div className="bbbbb">
        ...
        {etab.email&&<><h3>Email:</h3> <h4>{etab.email}</h4></>}
      </div>
      {etab.telephone && (
        <div className="bbbbb">
          <h3>Telephone:</h3> <h4>{etab.telephone}</h4>
        </div>
      )}
    </div>
    <div className="bbbbb">
    ...
    <Comment sett={sett} t={t} x={"etab"} id={etab._id} />
    {etab.Comments
      ? etab.Comments.map((x) => <Listcommet comments={x} id={etab._id} type={"etab"}sett={sett} t={t} />)
      : null}
  </div>
  )
}
